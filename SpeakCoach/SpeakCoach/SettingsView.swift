...
enum SettingsTab: String, CaseIterable, Identifiable {
    case appearance, guidance, teleprompter, external, browser, coaching
    ...
    var icon: String {
        ...
        case .coaching:   return "chart.line.uptrend.xyaxis"
        }
    }
}
...
struct SettingsView: View {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            HStack(spacing: 8) {
                                ...
                                Button("Restart") {
                                    SpeakCoachService.shared.browserServer.stop()
                                    SpeakCoachService.shared.browserServer.start()
    ...
    @State private var showClearCoachingConfirmation = false

    private var coachingTab: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Track your speaking metrics across sessions and get coaching feedback after each reading.")
                .font(.system(size: 11))
                .foregroundStyle(.secondary)

            Toggle(isOn: $settings.coachingEnabled) {
                Text("Enable Speech Coaching")
                    .font(.system(size: 13, weight: .medium))
            }
            .toggleStyle(.switch)
            .controlSize(.small)
            ...
            if settings.coachingEnabled {
                ...
                Toggle(isOn: $settings.showSessionReport) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Show Report After Session")
                            .font(.system(size: 13, weight: .medium))
                        Text("Displays a coaching report with metrics and tips when you finish reading.")
                            .font(.system(size: 11))
                            .foregroundStyle(.secondary)
                    }
                }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Custom Filler Words")
                        .font(.system(size: 13, weight: .medium))
                    Text("Add extra filler words to detect, separated by commas. Default fillers (um, uh, like, etc.) are always tracked.")
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
                    TextField("e.g. gonna, kinda, stuff", text: $settings.customFillerWords)
                        .textFieldStyle(.roundedBorder)
                        .font(.system(size: 12))
                }
                ...
                HStack(spacing: 10) {
                    ...
                    let sessionCount = SessionStore.shared.sessions.count
                    if sessionCount > 0 {
                        Text("\(sessionCount) session\(sessionCount == 1 ? "" : "s") recorded")
                            .font(.system(size: 11))
                            .foregroundStyle(.tertiary)
                    }
                    ...
                    if SessionStore.shared.sessions.count > 0 {
                        Button("Clear All Data") {
                            showClearCoachingConfirmation = true
                        }
                        .font(.system(size: 11))
                        .foregroundStyle(.red)
                        .buttonStyle(.plain)
            ...
            HStack(spacing: 6) {
                Image(systemName: "info.circle")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
                Text("Coaching requires Word Tracking or Voice-Activated mode (not Classic).")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
            }
        ...
        .alert("Clear All Coaching Data?", isPresented: $showClearCoachingConfirmation) {
            Button("Cancel", role: .cancel) {}
            Button("Clear All", role: .destructive) { SessionStore.shared.clearAll() }
        } message: {
            Text("This will permanently delete all session history and coaching data.")
        }
    }
