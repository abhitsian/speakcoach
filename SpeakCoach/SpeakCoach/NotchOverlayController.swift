...
class NotchOverlayController: NSObject {
    ...
    func show(text: String, hasNextPage: Bool = false, onComplete: (() -> Void)? = nil) {
        ...
        // Start speech coaching analytics
        if NotchSettings.shared.coachingEnabled && settings.listeningMode != .classic {
            let a = SpeechAnalytics()
            a.startSession(sourceText: text)
    ...
    func updateContent(text: String, hasNextPage: Bool) {
        ...
        // Reset segment tracking for new page but keep accumulated metrics
        analytics?.resetSegmentTracking()
    }
    ...
    func dismiss() {
        // Finalize coaching analytics before stopping the recognizer
        finalizeAnalytics()
    ...
    private func finalizeAnalytics() {
        guard let analytics else { return }
        if let session = analytics.finalize() {
    ...
    private func observeDismiss() {
        ...
        Timer.publish(every: 0.1, on: .main, in: .common)
            ...
            .sink { [weak self] _ in
                ...
                if let targetIndex = self.overlayContent.jumpToPageIndex {
                    ...
                    SpeakCoachService.shared.jumpToPage(index: targetIndex)
                }
            }
        ...
        Timer.publish(every: 0.1, on: .main, in: .common)
            ...
            .sink { [weak self] _ in
                ...
                // Finalize coaching analytics
                self.finalizeAnalytics()
