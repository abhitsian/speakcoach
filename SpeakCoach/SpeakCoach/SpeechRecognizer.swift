...
class SpeechRecognizer {
    ...
    /// Accumulated transcript text across recognition restarts
    var accumulatedTranscript: String = ""
    private var currentSegmentText: String = ""

    /// Full transcript including current partial recognition
    var fullTranscript: String {
        if currentSegmentText.isEmpty { return accumulatedTranscript }
        return accumulatedTranscript + (accumulatedTranscript.isEmpty ? "" : " ") + currentSegmentText
    }

    /// Analytics delegate for speech coaching
    weak var analyticsDelegate: SpeechAnalytics?
    ...
    /// Start recognition in meeting mode (no source text to match against)
    func startListening() {
        start(with: "")
    }
    ...
    func start(with text: String) {
        ...
        case .denied, .restricted:
            error = "Microphone access denied. Open System Settings → Privacy & Security → Microphone to allow SpeakCoach."
            ...
            AVCaptureDevice.requestAccess(for: .audio) { [weak self] granted in
                DispatchQueue.main.async {
                    if granted {
                        ...
                        self?.error = "Microphone access denied. Open System Settings → Privacy & Security → Microphone to allow SpeakCoach."
                    }
                }
            }
            return
    ...
    private func requestSpeechAuthAndBegin() {
        SFSpeechRecognizer.requestAuthorization { [weak self] status in
            DispatchQueue.main.async {
                ...
                default:
                    self?.error = "Speech recognition not authorized. Open System Settings → Privacy & Security → Speech Recognition to allow SpeakCoach."
    ...
    private func beginRecognition() {
        ...
        inputNode.installTap(onBus: 0, bufferSize: 1024, format: nil) { [weak self] buffer, _ in
            ...
            DispatchQueue.main.async {
                ...
                self?.analyticsDelegate?.recordAudioLevel(level)
            }
        }
        ...
        recognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest) { [weak self] result, error in
            ...
            if let result {
                ...
                DispatchQueue.main.async {
                    ...
                    // Commit finalized text to accumulated transcript
                    if result.isFinal {
                        self.accumulatedTranscript += (self.accumulatedTranscript.isEmpty ? "" : " ") + spoken
                        self.currentSegmentText = ""
                    }

                    // Feed transcription segments to analytics
                    self.analyticsDelegate?.recordSegments(result.bestTranscription.segments)
                }
            }
            if error != nil {
                DispatchQueue.main.async {
